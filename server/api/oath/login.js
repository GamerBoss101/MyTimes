...
export default class Login extends APIRoute {
    ...
    async post(req, res) {
        ...
        if(req.cookies.auth && req.app.get("session").getSessionUser(req.cookies.auth)) return res.status(400).json({ error: "Already logged in", status: 400 });
        ...
        let session = req.app.get("session").createSession(user.id);
